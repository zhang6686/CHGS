...
args = {
    'render_video_fps': 30,
    'render_video_width': 600,
    'render_video_height': 820,
...
def do_render_one_frame_no_gt(renderer, frame_idx, vertices, faces):
    if frame_idx % 100 == 0:
        print('processed', frame_idx, 'frames')
    #uniform_color = [220, 220, 220, 255]
    uniform_color = [135, 206, 235, 255]  # 天蓝色
    pose_camera = create_pose_camera(angle_deg=-2)
    pose_light = create_pose_light(angle_deg=-30)
...
def render_one_sequence_with_face(res_npz_path, output_dir, audio_path, model_folder="./smplx_models/", model_type='smplx', gender='NEUTRAL_2020', ext='npz', num_betas=300, num_expression_coeffs=100, use_face_contour=False, use_matplotlib=False, remove_transl=True):
    import smplx
    import torch
...
def render_one_sequence(res_npz_path, gt_npz_path, output_dir, audio_path, model_folder="./smplx_models/", model_type='smplx', gender='NEUTRAL_2020', ext='npz', num_betas=300, num_expression_coeffs=100, use_face_contour=False, use_matplotlib=False, remove_transl=True):
    import smplx
    import torch
    data_np_body = np.load(res_npz_path, allow_pickle=True)
...
def render_one_sequence_no_gt(res_npz_path, output_dir, audio_path, model_folder="./smplx_models/", model_type='smplx', gender='NEUTRAL_2020', ext='npz', num_betas=300, num_expression_coeffs=100, use_face_contour=False, use_matplotlib=False, remove_transl=True):
    import smplx
    import torch
    data_np_body = np.load(res_npz_path, allow_pickle=True)
    if not os.path.exists(output_dir):
...
def render_one_sequence_face_only(res_npz_path, output_dir, audio_path, model_folder="./smplx_models/", model_type='smplx', gender='NEUTRAL_2020', ext='npz', num_betas=300, num_expression_coeffs=100, use_face_contour=False, use_matplotlib=False, remove_transl=True):
    import smplx
    import torch
    data_np_body = np.load(res_npz_path, allow_pickle=True)
    if not os.path.exists(output_dir):
    ...
    pose1 = torch.zeros_like(pose).to(torch.float32).cuda()
    output1 = model(betas=beta, transl=transl, expression=expression, jaw_pose=jaw_pose, global_orient=pose1[:,:3], body_pose=pose1[:,3:21*3+3], left_hand_pose=pose1[:,25*3:40*3], right_hand_pose=pose1[:,40*3:55*3], leye_pose=pose1[:,69:72], reye_pose=pose1[:,72:75], return_verts=True)
    vertices_all = output1["vertices"].cpu().numpy()
    x_center = np.mean(vertices_all[:, :, 0])  # 计算X轴中心点
    vertices_centered = vertices_all.copy()  # 创建副本避免修改原数据
    vertices_centered[:, :, 0] -= x_center  # X轴居中

    # 第二步：应用缩放和Y轴偏移
    v1 = vertices_centered * 7  # 缩放7倍
    td = np.zeros_like(v1)
    # td[:, :, 1] = 11  
    td[:, :, 1] = 1.2  # Y轴偏移10个单位
    vertices_final = v1 - td  # 最终顶点位置

# 更新vertices_all变量
    vertices_all = vertices_final
    #x_center = (vertices_all[:, :, 0].min() + vertices_all[:, :, 0].max()) / 2
    #vertices_all[:, :, 0] -= x_center
    #v1 = output1["vertices"].cpu().numpy()*7
    #td = np.zeros_like(v1)
    #td[:, :, 1] = 10
    #vertices_all = v1 - td
    #vertices_all[:, :, 0] += 0.5  # 所有顶点X坐标+0.5
    if args['debug']:
        seconds = 1
    else:
        seconds = vertices_all.shape[0]//30
