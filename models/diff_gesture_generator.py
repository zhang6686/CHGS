...
class DiffGestureGenerator(nn.Module):
    def __init__(self, args, device='cuda'):
        ...
        if self.audio_model == 'wav2vec2':
            ...
            hubert_source = os.environ.get('HUBERT_MODEL_PATH', 'facebook/hubert-base-ls960')
            # Silence expected pretrained-weight warnings during HuBERT initialization.
            prev_verbosity = transformers_logging.get_verbosity()
            try:
                transformers_logging.set_verbosity_error()
                self.audio_encoder = HubertModel.from_pretrained(hubert_source)
            finally:
    ...
    def forward(self, motion_feat, audio_or_feat,
    ...
    def sample(self, audio_or_feat, prev_motion_feat=None, prev_audio_feat=None,
        ...
        if ret_traj:
            ...
            return torch.cat([results[0][:, :4], traj[0]], dim=1), motion_at_T, audio_feat
