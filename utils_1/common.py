...
def coef_dict_to_vertices(coef_dict, flame, rot_repr='aa', ignore_global_rot=False, flame_batch_size=512):
    ...
    return vert_list
def compute_loss(args, is_starting_sample, motion_coef_gt, noise, target, prev_motion_coef,  end_idx=None):
    ...
    if args.target == 'noise':
        ...
        if args.l_vert > 0 or args.l_vel > 0:
            verts_gt = motion_coef_gt[:, :, 330:333]
            verts_pred = target[:, :, 330:333]
            ...
            if not is_starting_sample and args.l_trans > 0:
                pose_trans = torch.cat([verts_gt[:, args.n_prev_motions - 3:args.n_prev_motions],
                                             verts_pred[:, args.n_prev_motions:args.n_prev_motions + 3]], dim=1)
                vel_pred = pose_trans[:, 1:] - pose_trans[:, :-1]
                accel_pred = vel_pred[:, 1:] - vel_pred[:, :-1]

                # will constrain x_{-2|0} ~ x_{1}
                loss_trans_vel = criterion_func(vel_pred[:, 2:4], vel_pred[:, 1:3], reduction='none')
                # will constrain x_{-3|0} ~ x_{2}
                loss_trans_accel = criterion_func(accel_pred[:, 1:], accel_pred[:, :-1],
                                                       reduction='none')

        # head pose
        if True:
            
            #head_pose_gt = motion_coef_gt[:, :, :165]
            #head_pose_pred = target[:, :, :165]
            head_pose_gt = motion_coef_gt
            head_pose_pred = target
    ...
    if loss_trans_vel is not None:
        vel_mask = mask[:, args.n_prev_motions:args.n_prev_motions + 2]
        accel_mask = mask[:, args.n_prev_motions:args.n_prev_motions + 3]
        loss_trans_vel = loss_trans_vel[vel_mask].mean()
        loss_trans_accel = loss_trans_accel[accel_mask].mean()
        loss_trans = loss_trans_vel + loss_trans_accel
    ...
    return loss_noise, loss_vert, loss_vel, loss_smooth, loss_trans, loss_head_angle, loss_head_vel, loss_head_smooth, \
           loss_head_trans
...
def _truncate_coef_dict(coef_dict, end_idx, pad_mode='zero'):
    batch_size = coef_dict['pose'].shape[0]
...
def truncate_motion_coef_and_audio(audio, motion_coef, n_motions, audio_unit=640, pad_mode='zero'):
    ...
    coef_dict = {
    'pose': motion_coef[..., :330],      # 取前 330 个元素，tar_pose_6d
    'trans': motion_coef[..., 330:333],  # 取接下来的 3 个元素，tar_trans
    'contact': motion_coef[..., 333:]     # 取最后的 4 个元素，tar_contact
            }

    # truncate coef dict
    coef_dict_trunc = _truncate_coef_dict(coef_dict, end_idx, pad_mode=pad_mode)
    motion_coef_trunc = torch.cat([coef_dict_trunc['pose'], coef_dict_trunc['trans'], coef_dict_trunc['contact']], dim=-1)
