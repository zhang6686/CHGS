...
class CustomDataset(Dataset):
    def dict_to_namespace(self, d):
        if isinstance(d, dict):
            return Namespace(**{k: self.dict_to_namespace(v) for k, v in d.items()})
        elif isinstance(d, list):
            return [self.dict_to_namespace(x) for x in d]
        else:
            return d
    def __init__(self, loader_type, augmentation=None, kwargs=None, build_cache=True):
        #self.args = args
        config_path = os.path.join(os.path.dirname(__file__), "..", "configs", "emage.yaml")
        with open(config_path, 'r') as f:
            config_dict = yaml.safe_load(f)
        args = self.dict_to_namespace(config_dict)
        ...
        self.max_length = int(args.pose_length * 1)
